...
grub-install --target=x86_64-efi --efi-directory=/boot/efi --no-nvram --bootloader-id=ubuntu # installs grub to EFI in /boot/efi/EFI/ubuntu
grub-install --target=x86_64-efi --efi-directory=/boot/efi --no-nvram --removable # installs grub to EFI fallback directory /boot/eft/EFI/BOOT
