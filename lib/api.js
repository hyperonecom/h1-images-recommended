...
const waitState = async (vm, states) => {
    vm = await vmApi.vmShow(vm._id).catch(async err => {
        console.log('Error in checking state of VM', err);
        await delay(120 * 1000);
        return vmApi.vmShow(vm._id);
    });
    if (ensureState(vm, states)) {
        console.log(`Required state achieved. Virtual Machine '${vm._id}' state is '${vm.state}'.`);
        return vm;
    }
    console.log(`Waiting. Virtual Machine '${vm._id}' state is '${vm.state}'.`);
    await delay(60 * 1000);
    return waitState(vm, states);
};
