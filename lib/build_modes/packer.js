...
const getConfig = (name, imageConfig, platformConfig, scope) => {
    if (name in imageConfig) {
        return scope in imageConfig[name] ? imageConfig[name][scope] : imageConfig[name];
    }
    return scope in platformConfig[name] ? platformConfig[name][scope] : platformConfig[name];
};
