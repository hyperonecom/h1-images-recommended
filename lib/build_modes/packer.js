...
const buildPackerImage = async (imageConfig, platformConfig, scope) => {
    ...
    const packer_var = [
        ...
        '-var', `project=${getter('project')}`,
    ];
    const packer_env = Object.fromEntries(
        Object
            .entries(process.env)
            .filter(([key]) => key.startsWith('HYPERONE_'))
    );
    const packer_argv = [
        'validate',
        ...packer_var,
    ...
    await runProcess('packer', packer_argv, { env: packer_env, suppressEmptyLines: true });

    packer_var.push([
        `-on-error=${platformConfig.onError}`,
    ]);
    const args = [
        'build',
        '-machine-readable',
    ...
    console.log(`Packer CMD: packer ${args.join(' ')}`);
    const output = await runProcess('packer', args, { env: packer_env, suppressEmptyLines: true });
    const match = output.match(/hyperone(,artifact,0,id,|:.*?ID: )([0-9a-f]+?)$/m);
    if (!match) {
